...
{
    int sockfd[2];
    Socketpair(AF_LOCAL, SOCK_DGRAM, 0, sockfd);

    pid_t pid = Fork();
    if (pid > 0)
    { // parent
        char tmp_parent_file[32] = "/tmp/parent_sock_XXXXXX";
        struct sockaddr_un sun;
        bzero(&sun, sizeof(sun));
        sun.sun_family = AF_LOCAL;
        strcpy(sun.sun_path, mktemp(tmp_parent_file));
        Bind(sockfd[0], (SA*)&sun, sizeof(sun));
    ...
    char tmp_child_file[32] = "/tmp/parent_sock_XXXXXX";
    struct sockaddr_un sun;
    bzero(&sun, sizeof(sun));
    sun.sun_family = AF_LOCAL;
    strcpy(sun.sun_path, mktemp(tmp_child_file));
    Bind(sockfd[1], (SA*)&sun, sizeof(sun));
