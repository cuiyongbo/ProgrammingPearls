...
{ 
    ...
    while(loops-- > 0)
    {
        for (size_t i = 0; i < item_count; i++)
        {
            ...
            rc = memcached_exist(memc, keys[i], strlen(keys[i]));
            ...
            if(rc == MEMCACHED_SUCCESS)
            {
                ...
                if(value == NULL)
                {
                    cout << "memcached_get(" << keys[i] << ") failed: " << memcached_strerror(memc, rc) << endl;
                }
                else
                {
                    cout << "memcached_get(" << keys[i] << ") succeeded: " << value << endl;
                }
                free(value);
                ...
                if (si == NULL)
                {
                    cout << "memcached_server_by_key failed: " << memcached_strerror(memc, rc) << endl;
                }
                else
                {

                    cout << "key: " << keys[i] << ", server: "<< memcached_server_name(si) << ":" << memcached_server_port(si) << endl;   
                }
            }
            else if(rc == MEMCACHED_NOTFOUND)
            {
                ...
                if(rc == MEMCACHED_SUCCESS)
                {
                    cout << "memcached_set(" << keys[i] << ") succeeded" << endl;
                }
                else
                {
                    cout << "memcached_set(" << keys[i] <<") failed: " << memcached_strerror(memc, rc) << endl;
                }
            }
            else
            {
                cout << "memcached_exist(" << keys[i] << ") failed: " << memcached_strerror(memc, rc) << endl;
            }
        }
        sleep(1 << 2);
    }
    ...
    if(rc == MEMCACHED_SUCCESS)
    {
        cout << "memcached_flush() succeeded" << endl;
    }
    else
    {
        cout << "memcached_flush() failed: " << memcached_strerror(memc, rc) << endl;
    }

    // remove keys
    for (size_t i = 0; i < item_count; i++)
    {
        rc = memcached_delete(memc, keys[i], strlen(keys[i]), 0);

        if(rc == MEMCACHED_SUCCESS)
        {
            cout << "memcached_delete(" << keys[i] << ") succeeded" << endl;
        }
        else
        {
            cout << "memcached_delete(" << keys[i] <<") failed: " << memcached_strerror(memc, rc) << endl;
        }
    }
