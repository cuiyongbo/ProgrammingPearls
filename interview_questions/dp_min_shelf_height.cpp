...
int Solution::minHeightShelves(vector<vector<int>>& books, int shelf_width)
{
    /*
        ...
        the first and second book onto the first shelf, the third book on the second shelf, and the fourth and 
    ...
    */
    ...
    // dp[i] = minHeightShelves(books[0:i])
    // dp[j] = min{dp[i-1]+max(h[i:j])}, 0< i <=j, sum(w[i:j]) <= shelf_width
