                                     ...
                                     const NodeID &start2)
{
    ...
    for (std::size_t added_nodes = 2; added_nodes < number_of_locations; ++added_nodes)
    {
        ...
        for (std::size_t id = 0; id < number_of_locations; ++id)
        {
            ...
            if (!visited[id])
            {
                const auto insert_candidate = GetShortestRoundTrip(id, dist_table, number_of_locations, route);

                BOOST_ASSERT_MSG(insert_candidate.first != INVALID_EDGE_WEIGHT, "shortest round trip is invalid");
