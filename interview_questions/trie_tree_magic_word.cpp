...
{
    if(m_trieTree.search(word)) return false;

    int len = (int) word.length();
    TrieNode* root = m_trieTree.root();
    ...
    i++; // skip the first different position
    for(auto node: prev->children)
    {
        if(find(node, i)) return true;
    }
    return false;
}
